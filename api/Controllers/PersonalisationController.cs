...
namespace IPKP___API.Controllers
{
  ...
  public class PersonalisationController : ControllerBase
  {
        ...
        public async Task<IActionResult> AddPersonalisationAsync(Personalisation_Design personalisation)
        {           
            try
            {
                var personalisationDesign = new Personalisation_Design
                {
                    ...
                    Design_Image_ID = personalisation.Design_Image_ID,
                };
                _IPKPRepository.Add(personalisationDesign);
                await _IPKPRepository.SaveChangesAsync();
                return Ok(personalisationDesign);
            }
            catch (Exception)
            {
                return BadRequest(new Response { Status = "Error", Message = "Internal Service Error, Please Contact Support." });
